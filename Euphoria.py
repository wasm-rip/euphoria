...
import threading, os, sys, time, json, re, hashlib, struct, uuid, difflib
...
class ChangeRecord:
def **init**(self, op_name, files):
self.op_name   = op_name
self.timestamp = datetime.now()
self.files     = files

```
def undo(self, log_cb=None):
    n = 0
    for e in self.files:
        try:
            Path(e["path"]).write_text(e["before"], encoding="utf-8")
            n += 1
            if log_cb: log_cb(f"  ↩  {Path(e['path']).name}")
        except Exception as ex:
            if log_cb: log_cb(f"  ERROR {Path(e['path']).name}: {ex}")
    return n

def redo(self, log_cb=None):
    n = 0
    for e in self.files:
        try:
            Path(e["path"]).write_text(e["after"], encoding="utf-8")
            n += 1
            if log_cb: log_cb(f"  ↪  {Path(e['path']).name}")
        except Exception as ex:
            if log_cb: log_cb(f"  ERROR {Path(e['path']).name}: {ex}")
...
dest.write_text(SHADER_CONTENT,encoding=“utf-8”)
...
f”  userData: \n  assetBundleName: \n  assetBundleVariant: \n”,
encoding=“utf-8”)
files = [{“path”: str(dest), “before”: before, “after”: SHADER_CONTENT}]
return True, f”Injected to {dest}”, files
...
patched=0; errors=[]; changed_files=[]
...
for mat_path in mat_files:
    try:
        ...
        txt2=re.sub(r'm_Shader:\s*\{[^}]*\}',
                    f'm_Shader: {{fileID: 4800000, guid: {shader_guid}, type: 3}}', txt)
        ...
        for tex in _WASM_TEX_DEFAULTS:
            if tex not in txt:
                ...
                changed=True
        if changed:
            changed_files.append({"path": str(mat_path), "before": src, "after": txt})
...
return patched, errors, changed_files
```

def fix_script_guids(project_path: Path, log_cb=None):
fixed=0; skipped=0; errors=[]; changed_files=[]
...
if new!=old:
changed_files.append({“path”: str(meta), “before”: old, “after”: new})
meta.write_text(new,encoding=“utf-8”)
if log_cb: log_cb(f”  Updated {cs.name}  →  {guid}”)
fixed+=1
else: skipped+=1
else:
new_content=(f”fileFormatVersion: 2\nguid: {guid}\nMonoImporter:\n”
f”  externalObjects: {{}}\n  serializedVersion: 2\n”
f”  defaultReferences: []\n  executionOrder: 0\n”
f”  icon: {{instanceID: 0}}\n  userData: \n”
f”  assetBundleName: \n  assetBundleVariant: \n”)
changed_files.append({“path”: str(meta), “before”: “”, “after”: new_content})
...
return fixed, skipped, errors, changed_files
...
if not m: continue
guid = m.group(1)
try: src = cs_path.read_text(encoding=“utf-8”, errors=“replace”)
...
if sfm: fields.add(sfm.group(1))
guid_map[guid] = {“class”: class_name, “fields”: fields, “path”: cs_path}
class_map[class_name.lower()] = guid
if log_cb: log_cb(f”  Script index: {len(guid_map)} scripts catalogued”)
...
if “MonoBehaviour:” not in block: continue
existing = re.search(
r’m_Script:\s*{fileID:\s*\d+,\s*guid:\s*([0-9a-f]{32}),\s*type:\s*\d+}’, block)
if existing:
eg = existing.group(1)
if eg != “0” * 32 and eg in guid_map: skipped += 1; continue
if eg not in guid_map and eg != “0” * 32: skipped += 1; continue
block_keys = set(re.findall(r’^\s{2,}(\w+):’, block, re.MULTILINE))
best_guid = None; best_score = 0
for g, info in guid_map.items():
score = 0
if info[“class”].lower() in block.lower(): score += 5
score += len(block_keys & info[“fields”])
if score > best_score: best_score = score; best_guid = g
if not best_guid or best_score < 1:
if log_cb: log_cb(f”  ⚠  {f.name}: MonoBehaviour with no match (score=0) — skipped”)
skipped += 1; continue
info = guid_map[best_guid]
new_ref = f’m_Script: {{fileID: 11500000, guid: {best_guid}, type: 3}}’
patched_block = re.sub(r’m_Script:\s*{[^}]*}’, new_ref, block, count=1)
if patched_block != block:
out = out.replace(block, patched_block, 1); hits += 1
if log_cb: log_cb(f”  ✓  {f.name}  →  {info[‘class’]}  (score {best_score})”)
if hits:
changed_files.append({“path”: str(f), “before”: src, “after”: out})
...
if not shader_m: skipped += 1; continue
cur_guid = shader_m.group(2)
if cur_guid in project_shaders: skipped += 1; continue
if cur_guid == “0000000000000000f000000000000000”: skipped += 1; continue
...
if score > best_score:
best_score = score; best_name = sname_lower
new_file_id, new_guid, new_type = “4800000”, guid, “3”
except Exception: pass
...
src, count=1)
changed_files.append({“path”: str(mat_path), “before”: src, “after”: patched_src})
...
_TMP_SIGNATURES = [
(“TextMeshPro”,     [“m_text:”,“m_fontAsset:”,“m_sharedMaterial:”,“m_isOrthographic:”]),
(“TextMeshProUGUI”, [“m_text:”,“m_fontAsset:”,“m_sharedMaterial:”,“m_raycastTarget:”]),
(“TMP_InputField”,  [“m_TextComponent:”,“m_Placeholder:”,“m_CaretBlinkRate:”]),
(“TMP_Dropdown”,    [“m_Template:”,“m_CaptionText:”,“m_ItemText:”]),
(“TMP_SubMesh”,     [“m_TextComponent:”,“m_sharedMaterial:”,“m_fontAsset:”,“m_isOrthographic:”]),
(“TMP_SubMeshUI”,   [“m_TextComponent:”,“m_sharedMaterial:”,“m_fontAsset:”,“m_raycastTarget:”]),
(“TMP_FontAsset”,   [“m_AtlasTextures:”,“m_AtlasWidth:”,“m_GlyphTable:”,“m_CharacterTable:”]),
(“TMP_SpriteAsset”, [“m_SpriteCharacterTable:”,“m_SpriteGlyphTable:”,“m_spriteSheet:”]),
...
if g in _TMP_GUIDS: return _TMP_GUIDS[g]
...
results.append({“file”: f.name, “path”: str(f.relative_to(project_path)),
“go_name”: go_name, “component”: comp_type, “line”: line_no})
if log_cb: log_cb(f”  [{comp_type}]  {go_name}  ←  {f.name}:{line_no}”)
except Exception as e:
errors.append(f”{f.name}: {e}”)
if log_cb: log_cb(f”  ERROR {f.name}: {e}”)
if log_cb:
...
def **init**(self,parent,text,command,width=160,height=38,**kw):
bg_color=kw.pop(“bg”,CONTENT)
super().**init**(parent,width=width,height=height,
...
def _draw(self):
    ...
    self.create_text(w//2,h//2,text=self._text,fill=TEXT,font=(FF_BODY(),fs(10),"bold"))
...
def **init**(self,parent,text,command,width=130,height=32,**kw):
bg_c=kw.pop(“bg”,CARD)
super().**init**(parent,width=width,height=height,
...
def _draw(self):
    ...
    RoundRect.draw(self,1,1,w-1,h-1,r=6,fill=CARD_HOV if self._hover else "",outline=col,width=1)
    self.create_text(w//2,h//2,text=self._text,
                     fill=TEXT if self._hover else TEXT_MID,font=(FF_BODY(),fs(9)))
```

class ToggleSwitch(tk.Canvas):
def **init**(self,parent,value=True,on_change=None,**kw):
super().**init**(parent,width=46,height=24,highlightthickness=0,**kw)
...
def **init**(self,parent,min_val,max_val,value,on_change=None,
width=200,label_fmt=”{:.0f}”,**kw):
super().**init**(parent,width=width,height=28,highlightthickness=0,**kw)
...
def _draw(self):
    self.delete("all"); w=self._sw; cx=int(8+(w-16)*self._frac())
    ...
    self.create_text(w//2,24,text=self._fmt.format(self._val),fill=TEXT_DIM,font=(FF_BODY(),fs(7)))

def _x_to_val(self,x):
    w=self._sw; frac=max(0,min(1,(x-8)/(w-16)))
    raw=self._min+frac*(self._max-self._min)
    if isinstance(self._min,int) and isinstance(self._max,int): return int(round(raw))
    return round(raw,2)
...
def **init**(self,parent,width=400,**kw):
super().**init**(parent,width=width,height=6,highlightthickness=0,**kw)
self._width=width; self._pct=0; self._draw()

```
def _draw(self):
    self.delete("all"); w=self._width
    self.create_rectangle(0,0,w,6,fill=BORDER,outline="")
    fill_w=int(w*self._pct)
    if fill_w>0: self.create_rectangle(0,0,fill_w,6,fill=ACCENT,outline="")

def set(self,pct): self._pct=max(0,min(1,pct)); self._draw()
```

class LogBox(tk.Frame):
def **init**(self,parent,height=120,**kw):
super().**init**(parent,bg=CARD,**kw)
self._txt=tk.Text(self,height=6,bg=”#060c18”,fg=TEXT_MID,
font=(FF_MONO(),fs(8)),wrap=“word”,state=“disabled”,relief=“flat”,bd=0,
highlightthickness=1,highlightbackground=BORDER,insertbackground=ACCENT)
...
def _build(self):
    ...
    self._file_menu = tk.OptionMenu(top, self._file_var, "")
    self._file_menu.configure(bg=CARD, fg=TEXT, activebackground=ACCENT,
                               activeforeground=TEXT, highlightthickness=0,
                               font=(FF_MONO(), fs(8)), bd=0, relief="flat")
    self._file_menu["menu"].configure(bg=CARD, fg=TEXT, font=(FF_MONO(), fs(8)))
    self._file_menu.pack(side="left")
    self._file_var.trace_add("write", self._on_file_change)

    self._stat = tk.Label(top, text="", fg=TEXT_DIM, bg=CARD, font=(FF_MONO(), fs(7)))
    self._stat.pack(side="right")

    txt_frame = tk.Frame(self, bg=CARD)
    txt_frame.pack(fill="both", expand=True, padx=8, pady=(2, 6))

    self._txt = tk.Text(txt_frame, bg="#04080f", fg=TEXT_MID,
                        font=(FF_MONO(), fs(8)), wrap="none",
                        state="disabled", relief="flat", bd=0,
                        highlightthickness=1, highlightbackground=BORDER,
                        insertbackground=ACCENT)
    xsb = tk.Scrollbar(txt_frame, orient="horizontal", command=self._txt.xview, bg=BORDER)
    ysb = tk.Scrollbar(txt_frame, orient="vertical",   command=self._txt.yview, bg=BORDER)
    self._txt.configure(xscrollcommand=xsb.set, yscrollcommand=ysb.set)
    self._txt.grid(row=0, column=0, sticky="nsew")
    ysb.grid(row=0, column=1, sticky="ns")
    xsb.grid(row=1, column=0, sticky="ew")
    txt_frame.rowconfigure(0, weight=1); txt_frame.columnconfigure(0, weight=1)

    self._txt.tag_configure("add", background="#0d2a1a", foreground="#4dff88")
    self._txt.tag_configure("rem", background="#2a0d10", foreground="#ff6070")
    self._txt.tag_configure("hdr", background="#0d1830", foreground=ACCENT)
    self._txt.tag_configure("lnum", foreground=TEXT_DIM)
    self._txt.tag_configure("ctx", foreground="#3a4a60")

def load(self, record):
    self._record = record
    names = [f"[{i}] {Path(e['path']).name}" for i, e in enumerate(record.files)]
    menu = self._file_menu["menu"]
    menu.delete(0, "end")
    if not names:
        self._file_var.set("no files changed")
        self._render("")
        return
    for n in names:
        menu.add_command(label=n, command=lambda v=n: self._file_var.set(v))
...
def clear(self):
    ...
    self._render("")
    self._stat.configure(text="")

def _on_file_change(self, *_):
    if not self._record: return
    val = self._file_var.get()
    try: idx = int(val.split("]")[0].lstrip("["))
    except Exception: idx = 0
    self._render(self._record.make_diff(idx))

def _render(self, diff_text):
    self._txt.configure(state="normal")
    self._txt.delete("1.0", "end")
    if not diff_text:
        self._txt.insert("end", "  (no changes)\n", "ctx")
        self._txt.configure(state="disabled")
        self._stat.configure(text="")
        return
    adds = rems = lnum = 0
    for line in diff_text.splitlines():
        ...
        if line.startswith("---") or line.startswith("+++"):
            ...
            self._txt.insert("end", ln, "lnum"); self._txt.insert("end", line + "\n", "ctx")
    self._txt.configure(state="disabled")
    self._stat.configure(text=f"+{adds}  -{rems}",
                          fg="#4dff88" if adds >= rems else "#ff6070")
```
...
def _slide_in(self):
    ...
    if not moved: self._cur_y+=(self.SPEED if going_down else -self.SPEED)
    else: self._cur_y=self._tgt_y
    ...
    if not moved: self._after=self._root.after(12,self._slide_in)
    else: self._after=self._root.after(SETTINGS.get("toast_duration",3500),self._start_fade)
...
def **init**(self,parent,on_close):
super().**init**(parent,bg=”#0d1520”)
...
def **init**(self,parent,projects,on_pick,on_cancel):
super().**init**(parent,bg=”#080e18”)
self.place(relx=0,rely=0,relwidth=1,relheight=1)
...
def _draw_items(self):
    ...
    self.create_text(self.W//2,wh-20,text="v2.1",fill=TEXT_DIM,font=(FF_BODY(),fs(8)))
...
def **init**(self,parent,icon,title,subtitle,btn_label,on_execute,**kw):
super().**init**(parent,bg=CARD,highlightthickness=1,highlightbackground=BORDER,**kw)
...
```
    ...
    self._diff=DiffViewer(self)

    self._expand_btn=tk.Label(self,text="▸ log / diff",fg=TEXT_DIM,bg=CARD,
...
def _toggle_log(self,_=None):
    ...
    if self._expanded:
        ...
        self._log.pack(fill="x",padx=dp(18),pady=(0,4))
        self._diff.pack(fill="x",padx=dp(18),pady=(0,dp(8)),ipady=80)
        self._expand_btn.configure(text="▾ log / diff")
    else:
        self._progress.pack_forget(); self._log.pack_forget(); self._diff.pack_forget()
        self._expand_btn.configure(text="▸ log / diff")
...
def _run(self):
    ...
    self._log.clear(); self._diff.clear(); self._progress.set(0)
    self._on_execute(self._log_line, self._done)

def _log_line(self,msg):
    self.after(0,lambda:self._log.append(msg))

def _done(self,success,summary,record=None):
    self._running=False
    if record:
        push_change(record)
        self.after(0,lambda:self._diff.load(record))
    def _upd():
        self._btn.set_text("⚡  Execute")
        self._progress.set(1.0 if success else 0.3)
        self._status_lbl.configure(text="done" if success else "error",
...
def **init**(self,parent,icon,title,subtitle,btn_label,on_execute,on_scan=None,**kw):
super().**init**(parent,icon,title,subtitle,btn_label,on_execute,**kw)
...
def _scan_done(self,success,summary,record=None):
    self._finish(success,summary)
...
def **init**(self, parent, toast, **kw):
super().**init**(parent, bg=CONTENT, **kw)
self._toast = toast

```
    hdr = tk.Frame(self, bg=CONTENT)
    hdr.pack(fill="x", padx=dp(28), pady=(dp(24), 0))
    tk.Label(hdr, text="History", fg=TEXT, bg=CONTENT,
             font=(FF_TITLE(), fs(18), "bold")).pack(side="left", anchor="w")
    btn_row = tk.Frame(hdr, bg=CONTENT)
    btn_row.pack(side="right", anchor="e")
    OutlineButton(btn_row, "↩ Undo", self._do_undo, width=90, height=28, bg=CONTENT).pack(side="left", padx=(0,6))
    OutlineButton(btn_row, "↪ Redo", self._do_redo, width=90, height=28, bg=CONTENT).pack(side="left")

    tk.Label(self, text="Every fix is recorded — undo/redo restores files to their previous state",
    ...
    panes = tk.PanedWindow(self, orient="horizontal", bg=CONTENT, sashwidth=4, sashrelief="flat")
    panes.pack(fill="both", expand=True, padx=dp(28), pady=(dp(14), dp(14)))

    list_frame = tk.Frame(panes, bg=CARD, highlightthickness=1, highlightbackground=BORDER)
    panes.add(list_frame, minsize=200)
    tk.Label(list_frame, text="OPERATIONS", fg=TEXT_DIM, bg=CARD,
             font=(FF_BODY(), fs(7)), padx=12, pady=6).pack(anchor="w")
    tk.Frame(list_frame, bg=BORDER, height=1).pack(fill="x")
    self._listbox = tk.Listbox(list_frame, bg=CARD, fg=TEXT_MID,
                               selectbackground=ACCENT_GL, selectforeground=TEXT,
                               font=(FF_MONO(), fs(8)), relief="flat", bd=0,
                               highlightthickness=0, activestyle="none")
    self._listbox.pack(fill="both", expand=True)
    self._listbox.bind("<<ListboxSelect>>", self._on_select)

    diff_frame = tk.Frame(panes, bg=CARD, highlightthickness=1, highlightbackground=BORDER)
    panes.add(diff_frame, minsize=300)
    self._diff_viewer = DiffViewer(diff_frame)
...
def _refresh(self):
    ...
    for rec in reversed(_undo_stack):
        ts = rec.timestamp.strftime("%H:%M:%S")
        n = len(rec.files)
        self._listbox.insert("end", f"  {ts}  {rec.op_name}  ({n} file{'s' if n!=1 else ''})")
    for rec in reversed(_redo_stack):
        self._listbox.insert("end", f"  [redo] {rec.op_name}")
...
def _on_select(self, _):
    ...
    if not sel: return
    idx = sel[0]
    if idx < len(_undo_stack):
        self._diff_viewer.load(list(reversed(_undo_stack))[idx])

def _do_undo(self):
    if not _undo_stack:
        self._toast.show("Nothing to undo", "No operations in history", success=False); return
    rec = _undo_stack.pop()
    _redo_stack.append(rec)
    rec.undo()
    self._toast.show("Undone", f"{rec.op_name} — {len(rec.files)} file(s) restored", success=True)
    self._refresh()

def _do_redo(self):
    if not _redo_stack:
        self._toast.show("Nothing to redo", "Redo stack is empty", success=False); return
    rec = _redo_stack.pop()
    _undo_stack.append(rec)
    rec.redo()
    self._toast.show("Redone", f"{rec.op_name} — {len(rec.files)} file(s) re-applied", success=True)
    self._refresh()
...
def **init**(self,parent,toast,history_panel_ref=None,**kw):
super().**init**(parent,bg=CONTENT,**kw)
self._toast=toast; self._project=None; self._overlay=None
self._history_ref=history_panel_ref
...
def _build_cards(self):
    ...
    for i,(key,icon,title,sub,btn,fn) in enumerate(fixes):
        ...
        if key=="shader":
            card=TMPFixCard(self._scroll_frame,icon,title,sub,btn,fn,on_scan=self._exec_tmp_scan)
        else:
...
def _notify_history(self):
    if self._history_ref:
        try: self._history_ref.refresh()
        except Exception: pass
...
def _exec_tmp_scan(self,log,done):
    ...
    def _w():
        ...
        patched, mat_errors, _ = apply_wasm_shader_to_tmp_materials(self._project, log)
        ...
        summary = f"Patched {patched} material(s)  ·  Found {len(results)} TMP object(s)"
...
def _exec_broken_shaders(self, log, done):
    ...
    def _w():
        ...
        fixed, skipped, errors, changed_files = fix_broken_shaders(self._project, log)
        summary = f"Fixed {fixed} material(s)  ·  {skipped} already OK"
        if errors: summary += f"  ·  {len(errors)} error(s)"
        record = ChangeRecord("Broken Shader Fix", changed_files) if changed_files else None
        if fixed > 0: log("\n  Reimport Assets in Unity to apply shader changes.")
        self._notify_history()
        done(fixed > 0 or not errors, summary, record)
    threading.Thread(target=_w, daemon=True).start()

def _exec_shader(self,log,done):
    def _w():
        log("  Injecting full TMP-compatible shader...")
        ok, msg, changed_files = inject_shader(self._project)
        record = ChangeRecord("TMP Shader Inject", changed_files) if changed_files else None
        if ok: self.after(600, self._show_tutorial)
        self._notify_history()
        done(ok, msg, record)
    if not self._require_project(): return
    threading.Thread(target=_w,daemon=True).start()
...
def _exec_guid(self,log,done):
    if not self._require_project(): return
    def _w():
        fixed, skipped, errors, changed_files = fix_script_guids(self._project, log)
        summary = f"Updated {fixed} GUIDs, skipped {skipped}, {len(errors)} error(s)"
        record = ChangeRecord("Script GUID Stability", changed_files) if changed_files else None
        self._notify_history()
        done(True, summary, record)
    threading.Thread(target=_w, daemon=True).start()
...
def _exec_ref_fix(self, log, done):
    ...
    def _w():
        ...
        fixed, skipped, errors, changed_files = fix_missing_script_refs(self._project, log)
        summary = f"Re-linked {fixed} reference(s)  ·  {skipped} skipped"
        if errors: summary += f"  ·  {len(errors)} error(s)"
        record = ChangeRecord("Missing Reference Fix", changed_files) if changed_files else None
...
def **init**(self,parent,rebuild_cb,**kw):
super().**init**(parent,bg=CONTENT,**kw)
...
def _card(self):
    ...
    c.pack(fill="x",pady=(0,dp(8))); return c
...
def _row(self,card,label,hint=""):
    ...
    ctrl=tk.Frame(r,bg=CARD); ctrl.pack(side="right",anchor="e"); return ctrl
...
def _build_appearance(self):
    ...
    for key,label,default in [("title","Title Font","Trebuchet MS"),
                               ("body","Body Font","Segoe UI"),("mono","Mono Font","Consolas")]:
        ...
        tk.Label(row,text=f"default: {default}",fg=TEXT_DIM,bg=CARD,font=(FF_BODY(),fs(7))).pack(side="left")
...
def _pick_accent(self):
    ...
    if col and col[1]:
        ...
        apply_theme(accent_override=hex_c); self._rebuild()

def _reset_accent(self):
    SETTINGS["accent_color"]=""; apply_theme(); self._rebuild()
...
def _build_behaviour(self):
    ...
    self._td_slider=SliderWidget(td_row,1000,8000,SETTINGS.get("toast_duration",3500),
...
def _reset_defaults(self):
    ...
    apply_theme(); save_settings(SETTINGS); self._rebuild()
```
...
tk.Label(tl,text=“UAR Gap Filler  ·  v2.1”,fg=TEXT_MID,bg=CARD,font=(FF_BODY(),fs(8))).pack(anchor=“w”)
...
“to reference it. Fixes red boxes and TMP material crashes.”),
...
“Indexes every .shader file in the project and remaps materials by property-signature matching.”),
(“Script GUID Stability”,“GUID · MonoScript · Prefab · Meta”,“ACTIVE”,SUCCESS,
“Derives script asset GUIDs deterministically from MD5(namespace+classname). “
“All m_Script references in Prefabs/Scenes resolve across re-exports.”),
...
“all-zero m_Script GUID. Scores candidates by serialised-field overlap and class-name hints.”),
]
...
def _build(self):
    ...
    history  = HistoryPanel(cw, self._toast)
    fixes    = UnityFixesPanel(cw, self._toast, history_panel_ref=history)
    settings = SettingsPanel(cw, rebuild_cb=self.rebuild)
    about    = AboutPanel(cw)

    self._panels={"unity_fixes":fixes,"history":history,"settings":settings,"about":about}
